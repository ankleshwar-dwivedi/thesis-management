...
function getFacultyNamesOfSameDepartment($conn, $studentEmail, $includeNullOption = false) {
    $names = "";
    ...
    if ($studentDeptResult->num_rows > 0) {
        ...
        if ($includeNullOption) {
            $names .= '<option value="">Select...</option>';
        }
...
<body>
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <select id="co_advisor" name="co_advisor" >
            <?php echo getFacultyNamesOfSameDepartment($conn, $_SESSION["email"],true); ?>
        </select><br>
